package com.wdz.utils.codelagoon;
...
public class BinUtils {
	...
	public final static short readInt16LE(byte[] data, int ofs) {
		return (short) (((data[ofs + 1] & 0xff) << 8) | (data[ofs] & 0xff));
	}
	...
	public final static short readInt16BE(byte[] data, int ofs) {
		return (short) (((data[ofs] & 0xff) << 8) | (data[ofs + 1] & 0xff));
	}
	...
	public final static int readInt32LE(byte[] data, int ofs) {
		return (data[ofs + 3] << 24) | ((data[ofs + 2] & 0xff) << 16) | ((data[ofs + 1] & 0xff) << 8)
				| (data[ofs] & 0xff);
	}
	...
	public final static int readInt32BE(byte[] data, int ofs) {
		return (data[ofs] << 24) | ((data[ofs + 1] & 0xff) << 16) | ((data[ofs + 2] & 0xff) << 8)
				| (data[ofs + 3] & 0xff);
	}
	...
	public final static long readInt64LE(byte[] data, int ofs) {
		return (readInt32LE(data, ofs) & 0x0ffffffffL) | ((long) readInt32LE(data, ofs + 4) << 32);
	}
	...
	public final static long readInt64BE(byte[] data, int ofs) {
		return (readInt32BE(data, ofs + 4) & 0x0ffffffffL) | ((long) readInt32BE(data, ofs) << 32);
	}
	...
	public static abstract class IntRW {
		...
		public final static IntRW LE = new IntRW() {
			public void writeInt16(short value, byte[] data, int ofs) {
				writeInt16LE(value, data, ofs);
			}

			public void writeInt32(int value, byte[] data, int ofs) {
				writeInt32LE(value, data, ofs);
			}

			public void writeInt64(long value, byte[] data, int ofs) {
				writeInt64LE(value, data, ofs);
			}

			public short readInt16(byte[] data, int ofs) {
				return readInt16LE(data, ofs);
			}

			public int readInt32(byte[] data, int ofs) {
				return readInt32LE(data, ofs);
			}

			public long readInt64(byte[] data, int ofs) {
				return readInt64LE(data, ofs);
			}
		};

		public final static IntRW BE = new IntRW() {
			public void writeInt16(short value, byte[] data, int ofs) {
				writeInt16BE(value, data, ofs);
			}

			public void writeInt32(int value, byte[] data, int ofs) {
				writeInt32BE(value, data, ofs);
			}

			public void writeInt64(long value, byte[] data, int ofs) {
				writeInt64BE(value, data, ofs);
			}

			public short readInt16(byte[] data, int ofs) {
				return readInt16BE(data, ofs);
			}

			public int readInt32(byte[] data, int ofs) {
				return readInt32BE(data, ofs);
			}

			public long readInt64(byte[] data, int ofs) {
				return readInt64BE(data, ofs);
			}
		};
	...
	public static int hexDump(byte[] data, PrintStream ps, int bytesPerLine, int bytesPerGroup) {
		return hexDump(data, 0, data.length, ps, bytesPerLine, bytesPerGroup);
	}

	public static int hexDump(byte[] data, int ofs, int len, PrintStream ps, int bytesPerLine, int bytesPerGroup) {
		return hexDump(new ByteArrayInputStream(data, ofs, len), ps, -1, bytesPerLine, bytesPerGroup);
	}

	public static int hexDump(BytePtr bp, PrintStream ps, int bytesPerLine, int bytesPerGroup) {
		return hexDump(new ByteArrayInputStream(bp.buf, bp.ofs, bp.len), ps, -1, bytesPerLine, bytesPerGroup);
	}

	public static int hexDump(InputStream is, PrintStream ps, int maxRead, int bytesPerLine, int bytesPerGroup) {
		if (1 > bytesPerLine) {
			bytesPerLine = 16;
		}
		if (1 > bytesPerGroup) {
