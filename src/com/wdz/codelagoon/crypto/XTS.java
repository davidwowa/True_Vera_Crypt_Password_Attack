 ...
 */
public class XTS implements Cloneable {
	BlockCipher cipher1;
	BlockCipher cipher2;
	 ...
	 * 
	 * @param cipher1
	 *            The first block cipher instance.
	 * @param cipher2
	 *            The second block cipher instance.
	 * @throws TCLibException
	 *             If the ciphers are not of identical types.
	 */
	public XTS(BlockCipher cipher1, BlockCipher cipher2) throws TCLibException {
		if (cipher1.blockSize() != BLOCK_SIZE || !cipher1.name().equals(cipher2.name())) {
	 ...
	 *            Data buffer.
	 * @param ofs
	 *            Where to start reading/writing in the buffer.
	 * @param len
	 *            How many bytes to process.
	 * @param startDataUnit
	 *            Block (as in volume) number.
	 * @param startBlock
	 *            Which sub-block to start at.
	 * @throws TCLibException
	 */
	public void process(byte[] buf, int ofs, int len, long startDataUnit, int startBlock) throws TCLibException {
		if (0 > ofs || 0 > len || 0 != len % BLOCK_SIZE || 0 > startDataUnit || 0 > startBlock) {
			throw new TCLibException();
		}
		...
		while (blockCount > 0) {
			int endBlock = blockCount < BLOCKS_PER_DATA_UNIT ? startBlock + blockCount : BLOCKS_PER_DATA_UNIT;
			...
			for (int block = startBlock, wi = w_vals_end; block < endBlock; block++, wi -= 2, ofs += SIZEOF_LONG * 2) {
	...
	final static String[][] TEST_VECTORS = {
			// IEEE 1619 vector 10
			{ "2718281828459045235360287471352662497757247093699959574966967627", // key
																					// 1
					"3141592653589793238462643383279502884197169399375105820974944592", // key
																						// 2
					"00000000000000ff", // data unit
					"0", // block #
					"000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f" + // plain
																							// text
							...
							+ "c0c1c2c3c4c5c6c7c8c9cacbcccdcecfd0d1d2d3d4d5d6d7d8d9dadbdcdddedf"
							+ "e0e1e2e3e4e5e6e7e8e9eaebecedeeeff0f1f2f3f4f5f6f7f8f9fafbfcfdfeff",
					"1c3b3a102f770386e4836c99e370cf9bea00803f5e482357a4ae12d414a3e63b" + // cipher
																							// text
			...
			{ "2718281828459045235360287471352662497757247093699959574966967627",
					"3141592653589793238462643383279502884197169399375105820974944592", "000000ffffffffff", "0",
					"000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"
							+ "202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f"
							+ "404142434445464748494a4b4c4d4e4f505152535455565758595a5b5c5d5e5f"
							+ "606162636465666768696a6b6c6d6e6f707172737475767778797a7b7c7d7e7f"
