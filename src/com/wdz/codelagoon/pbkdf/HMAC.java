...
import com.wdz.codelagoon.hash.RIPEMD160;
...
public class HMAC implements Hash.Producer {
	 ...
	 *            The key material.
	 * @param ofs
	 *            Where the key material is stored.
	 * @param len
	...
	public void erase() {
		if (null != this.k_opad)
			Arrays.fill(this.k_opad, (byte) 0);
		if (null != this.k_ipad)
			Arrays.fill(this.k_ipad, (byte) 0);
		if (null != this.hfnc)
			this.hfnc.erase();
	}
	...
	public void test() throws Throwable {
		final byte[][] REF_KEYS_RIPEMD160 = { BinUtils.hexStrToBytes("cc00112233445566778899aabbccddeeff01234567"),
				BinUtils.hexStrToBytes("cc0123456789abcdeffedcba987654321000112233"), };
		final byte[][] REF_DATA_RIPEMD160 = { "message digest".getBytes(),
				"12345678901234567890123456789012345678901234567890123456789012345678901234567890".getBytes() };
		final byte[][] REF_HASHES_RIPEMD160 = { BinUtils.hexStrToBytes("f83662cc8d339c227e600fcd636c57d2571b1c34"),
		...
		for (int i = 0; i < REF_KEYS_RIPEMD160.length; i++) {
			...
			hmac.update(REF_DATA_RIPEMD160[i], 0, REF_DATA_RIPEMD160[i].length);
			...
			if (!BinUtils.arraysEquals(hash, 1, REF_HASHES_RIPEMD160[i], 0, REF_HASHES_RIPEMD160[i].length)) {
				throw new Exception();
			}

			if ((hash[0] & 0x0ff) != 0xbb)
				throw new Exception();
			if ((hash[hash.length - 1] & 0x0ff) != 0xee)
				throw new Exception();
		}

		///////////////////////////////////////////////////////////////////////

		final byte[][] REFEX_HASHES = { BinUtils.hexStrToBytes("44d86b658a3e7cbc1a2010848b53e35c917720ca"),
		...
		final byte[][] REFEX_DATA = { "".getBytes(), "a".getBytes(), "0123456789\u0000\u0000\u0000".getBytes(),
				BinUtils.hexStrToBytes("baadbeef") };
		final byte[][] REFEX_KEYS = { "".getBytes(), "a".getBytes(),
		...
		for (byte[] rxdata : REFEX_DATA) {
			for (byte[] rxkey : REFEX_KEYS) {
				...
				if (!BinUtils.arraysEquals(hash, 0, REFEX_HASHES[hidx++], 0, hash.length)) {
					throw new Exception();
				}
			}
		}
